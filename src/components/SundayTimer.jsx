...
const SundayTimer = () => {
  const [timeRemaining, setTimeRemaining] = useState(calculateTimeRemaining(0, 10)); // Sunday at 10 AM

  useEffect(() => {
    const timer = setInterval(() => {
      const newTimeRemaining = calculateTimeRemaining(0, 10); // Sunday at 10 AM
      setTimeRemaining(newTimeRemaining);

      if (newTimeRemaining.days === 0 && newTimeRemaining.hours === 0 && newTimeRemaining.minutes === 0 && newTimeRemaining.seconds === 0) {
        // Timer reached zero, do something for Sunday service
        clearInterval(timer);
      }
    }, 1000);
